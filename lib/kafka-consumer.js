 ...
 */
 KafkaConsumer.prototype.consumeFromToppar = function(topic, partition, cb, options = {}) {
  ...
  if(typeof options.numberOfMessages !== 'number' && typeof options.numberOfMessages !== 'undefined') {
    throw new TypeError('options.numberOfMessages must be a number or undefined');
  }
  if(typeof options.timeoutMs !== 'number' && typeof options.timeoutMs !== 'undefined') {
    throw new TypeError('options.timeoutMs must be a number or undefined');
  }
  if(typeof options.onlyApplyTimeoutToFirstMessage !== 'boolean' && typeof options.onlyApplyTimeoutToFirstMessage !== 'undefined') {
    throw new TypeError('options.onlyApplyTimeoutToFirstMessage must be a boolean or undefined');
  }

  var numberOfMessages = options.numberOfMessages ?? 1;
 ...
 * @see consume
 */
KafkaConsumer.prototype._consumeNumOfPartition = function(timeoutMs, numMessages, topic, partition, cb, onlyApplyTimeoutToFirstMessage = false) {
  var self = this;
