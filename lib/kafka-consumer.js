...
KafkaConsumer.prototype.addQueueNotEmptyCallback = function(topicPartition, cb) {
  this._client.configureQueueNotEmptyCallback(topicPartition, cb, true);
  return this;
};

KafkaConsumer.prototype.removeQueueNotEmptyCallback = function(topicPartition, cb) {
  this._client.configureQueueNotEmptyCallback(topicPartition, cb, false);
  return this;
};
...
KafkaConsumer.prototype.rebalanceProtocol = function() {
  var protocol = this._client.rebalanceProtocol();
  if (protocol === '') {
    return null;
  }
  return protocol;
 ...
 * @param {object} options - Optional settings for consume
 */
 KafkaConsumer.prototype.consumeFromToppar = function(topic, partition, cb, options = {}) {
  ...
  if (typeof options.numberOfMessages !== 'number' && options.numberOfMessages !== undefined) {
    throw new TypeError('options.numberOfMessages must be a number or undefined');
  }
  if (typeof options.timeoutMs !== 'number' && options.timeoutMs !== undefined) {
    throw new TypeError('options.timeoutMs must be a number or undefined');
  }
  if (typeof options.onlyApplyTimeoutToFirstMessage !== 'boolean' && options.onlyApplyTimeoutToFirstMessage !== undefined) {
    throw new TypeError('options.onlyApplyTimeoutToFirstMessage must be a boolean or undefined');
  }

  var numberOfMessages = options.numberOfMessages === undefined ? 1 : options.numberOfMessages;
  var defaultTimeoutMs = this._consumeTimeout !== undefined ? this._consumeTimeout : DEFAULT_CONSUME_TIME_OUT;
  var timeoutMs = options.timeoutMs === undefined ? defaultTimeoutMs : options.timeoutMs;
  var onlyApplyTimeoutToFirstMessage = options.onlyApplyTimeoutToFirstMessage === true;

  this._consumeNumOfPartition(timeoutMs, numberOfMessages, topic, partition, cb, onlyApplyTimeoutToFirstMessage);
};
